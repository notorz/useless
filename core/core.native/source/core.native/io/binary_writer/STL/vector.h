...
namespace useless
{
	namespace binary_write_helper
	{
		...
		void write_member( Archive& bw, const std::vector<U, Allocator>& val, unsigned long count, std::true_type )
		{
			bw.write( &val[ 0 ], sizeof( U ) * count );
		}

		template<typename Archive, typename U, typename Allocator>
		void write_member( Archive& bw, const std::vector<U, Allocator>& val, unsigned long count, std::false_type )
		{
		...
		template<typename Archive, typename U, typename Allocator>
		void write_member( Archive& bw, const std::vector<U, Allocator>& val, unsigned long count )
		{
			if( !val.empty() )
			{
		...
		template<typename Archive, typename U, typename Allocator>
		void invoke( Archive& bw, const std::vector<U, Allocator>& val )
		{
